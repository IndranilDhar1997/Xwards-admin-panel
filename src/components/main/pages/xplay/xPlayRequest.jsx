...
class XPlayRequest extends Component {
	...
	constructor(props) {
        super(props);
        this.state = { xPlayRequests: '', totalItemsCount: '', activePage: 1 }
    }

    componentDidMount() {
        AjaxService.get(Routes.GET_XPLAY_REQUESTS()+"?page="+this.state.activePage, function(response) {
            console.log(response);
            this.setState({ xPlayRequests: response.xplay, totalItemsCount: response.count});
    ...
    handlePageChange(pageNumber) {
        ...
        AjaxService.get(Routes.GET_XPLAY_REQUESTS()+"?page="+pageNumber, function(response) {
            this.setState({xPlayRequests: response.xplay, totalItemsCount: response.count});
        }.bind(this), function(error) {
            MainNavBar__Toast('err', 'Some error happened while fetching details');
        })
    ...
    handleApprove = (e, requestId) => {
        ...
        AjaxService.post(Routes.ACCEPT_XPLAY_REQUEST(), data, function(response) {
            console.log(response);
    ...
    handleDecline = (e, requestId) => {
        e.preventDefault();
        let verify = window.prompt("Type 'DELETE' to delete this request.");
        if (!(verify === 'DELETE')) {
            return false;
        }
        AjaxService.delete(Routes.DELETE_XPLAY_REQUEST(requestId), function(response) {
            console.log(response);
            window.location.reload();
        }, function(error) {
            console.log(error);
	...
	render() {
		return (
            <div className="container-fluid margin-bottom-50" id= "requests">
                { this.state.xPlayRequests.length !== 0 && (
                    <React.Fragment>
                        <div className="row margin-top-20">
                            <div className="col col-12">
                                <h4 className="margin-bottom-20">Summary of Xplay Requests</h4>
                                <Table striped={true} bordered={true} hover={true} id="xplay-requests">
                                    <thead>
                                        <tr>
                                            <th>Email</th>
                                            <th>Phone</th>
                                    ...
                                    <tbody>
                                        {this.state.xPlayRequests.map(data => (
                                            <tr key={data.id}>
                                                <td>{data.email}</td>
                                                <td>{data.phone}</td>
                ...
                { this.state.xPlayRequests.length === 0 && (
                    <Card className="text-center margin-top-20">
                        <Card.Body>
                            <h5 className="text-secondary">No Xplay Requests</h5>
                        </Card.Body>
                    </Card>
                )}
            </div>
